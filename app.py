...
st.set_page_config(page_title="SoberSnap - 酔い止めVlog", page_icon="🍷", layout="centered")
...
if st.session_state.step == "start":
    ...
    st.markdown("---")
    st.session_state.party_title = st.text_input("今日の飲み会タイトル", "例：有嘉代と久々片町デート")
    
    if st.button("飲み会をスタートする 🚀", type="primary", use_container_width=True):
...
# --- 2. メイン画面（記録・修正・ミニテスト） ---
    ...
    if st.session_state.warning_level == 1:
        st.warning("⚠️ **【注意】少し酔いが回ってきたかも？ お水を一杯挟みましょう！**")
    elif st.session_state.warning_level >= 2:
        st.error("🚨 **【警告】判断力が低下しています！ ペースを落としてお水を飲んでください！**")

    st.markdown("---")

    st.subheader("📸 記録する")
    upload_type = st.radio("何を記録しますか？", ["乾杯・お酒", "おつまみ・料理"], horizontal=True)
    
    uploaded_file = st.file_uploader("写真を撮影（またはアップロード）", type=["jpg", "jpeg", "png"])
    
    if uploaded_file is not None:
        st.image(uploaded_file, caption="撮影された画像", use_container_width=True)
        
        if upload_type == "乾杯・お酒":
            default_name = random.choice(["生ビール (中ジョッキ)", "日本酒 (純米・一合)", "ハイボール", "赤ワイン"])
            st.write("▼ AIが自動予測しました（名前は自由に変更できます）")
            edited_drink_name = st.text_input("ドリンク名を確認・修正", value=default_name)
            
            if st.button("この内容で記録する", use_container_width=True):
                st.success(f"**{edited_drink_name}** を記録しました！")
                st.session_state.logs.append({
                    "type": "drink",
                    "name": edited_drink_name,
                    "alc": 5.0
                })
            ...
            default_dish = random.choice(["先付け・旬の小鉢盛り合わせ", "お造り盛り合わせ", "牛すじ煮込み", "旬の焼き魚"])
            st.write("▼ AIが自動予測しました（名前は自由に変更できます）")
            edited_dish_name = st.text_input("おつまみ名を確認・修正", value=default_dish)
            
            if st.button("このおつまみを記録する", use_container_width=True):
                st.success(f"おつまみ「**{edited_dish_name}**」を記録しました！")
                st.session_state.logs.append({
                    "type": "food",
                    "name": edited_dish_name
                })
...
# --- 3. ミニテスト画面 ---
    ...
    st.write("新しいお酒を楽しく安全に飲むために、簡単な計算テストに答えてください！")
    ...
    if st.button("回答する", type="primary"):
        try:
            if int(user_answer) == st.session_state.quiz_ans:
                ...
                st.session_state.warning_level = max(0, st.session_state.warning_level - 1)
            else:
                st.error(f"おっと、不正解！（正解は {st.session_state.quiz_ans}）ちょっと酔いが回ってきたかも…？")
...
# --- 4. エンド画面 ---
    ...
    st.success("本日のディナーの記録から、思い出のVログストーリーが生成されました。")
    ...
    if st.session_state.logs:
        for idx, log in enumerate(st.session_state.logs):
            if log["type"] == "drink":
                st.markdown(f"**[シーン {idx+1}] 乾杯・お酒** 🍺  \n-> `{log['name']}` を美味しくいただきました。")
            else:
                st.markdown(f"**[シーン {idx+1}] グルメ** 🍴  \n-> 美味しいおつまみ `{log['name']}` と共に最高の時間を堪能。")
    else:
